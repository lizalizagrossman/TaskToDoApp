...
class StorageManager {
    ...
    func delete(_ tasks: Task, from taskList: TaskList) {
        write {
            realm.delete(taskList.tasks)
        }
    }
    
    func edit(_ task: Task, taskNewValue: String, noteNewValue: String) {
        write {
            task.title = taskNewValue
            task.note = noteNewValue
        }
    }
